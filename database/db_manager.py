...
class DatabaseManager:
    ...
    def add_user(self, name, employee_id, email, department, face_encoding, image_path):
        """Add new user or reactivate if exists but inactive"""
        conn = self.get_connection()
        cursor = conn.cursor()
        ...
        try:
            ...
            cursor.execute('''
                SELECT id FROM users WHERE employee_id = ? AND is_active = 0
            ''', (employee_id,))
            result = cursor.fetchone()
            ...
            if result:
                ...
                cursor.execute('''
                    UPDATE users
                    SET name = ?, email = ?, department = ?, face_encoding = ?, image_path = ?, is_active = 1
                    WHERE id = ?
                ''', (name, email, department, face_encoding_blob, image_path, user_id))
                conn.commit()
                conn.close()
                return user_id
            else:
                # Insert new user normally
                cursor.execute('''
